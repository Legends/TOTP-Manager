...
The Avalonia desktop composition root now activates this same portable infrastructure graph. Until a reviewed macOS or Linux quick-unlock provider exists, it supplies `UnavailablePlatformQuickUnlock`, whose unrecognized provider identity cannot create or claim supported envelope metadata and whose operations return typed unavailable failures. Existing Windows quick-unlock wrappers therefore route safely to the mandatory master-password recovery path on those hosts. The WPF client continues to register the reviewed Windows Hello/TPM adapter; no envelope, vault, or preferences format changes are introduced by this composition change.

`AvaloniaStartupCoordinator` is the Avalonia initialization boundary. It loads only the allowlisted preferences contract before initializing authorization, preserves the locked/default state, and projects either password setup or password unlock without attempting an automatic credential operation. Preference failures and unexpected boundary exceptions produce a retryable, sanitized UI state that explicitly confirms encrypted data was not changed. Exception messages are not logged at this boundary because they may contain paths or user-controlled data. Cancellation propagates, and startup performs no envelope or vault write, so this slice adds no storage-format or migration impact.
