...
`AppPreferencesMapper` is the sole allowlisted bridge between mutable in-memory `IAppSettings` and `AppPreferencesV1`. It copies only reviewed preference fields and normalizes legacy or out-of-range values to codec-valid values. Neither live settings type contains authorization material, which makes the separation structural as well as behavioral.

`PortableSettingsService` is the active `ISettingsService` implementation backed only by `IAppPreferencesStore`. It maintains the stable mutable settings object expected by desktop consumers, applies portable preferences on first load, retries failed loads, and maps only allowlisted preference fields on save. Tests verify that the persisted `AppPreferencesV1` contract has no authorization, password, or wrapped-key property.
...
First-run setup preserves a typed existing-vault conflict when a candidate v2 key cannot verify a vault already present on disk. The Avalonia setup view reports that the existing encrypted data was not changed and does not misclassify the refusal as a password-policy error. The application never deletes or replaces the conflicting vault automatically; starting fresh requires an explicit operator action to move the development-era vault aside, while recovery requires its matching historical authorization material.

The Avalonia unlock and settings workflows obtain gate and configuration status from `IAuthorizationService.State`. Enabling platform quick unlock and replacing the recovery password require an explicit current-password prompt; cancellation fails closed before enrollment or password lifecycle work. The prompt does not retain or persist the supplied password. Live presentation and settings code has no dependency on the development-era authorization profile.

`AuthorizationState.SetConfiguration` derives configured state from envelope presence and the portable preferred-unlock preference. The desktop UI consumes `AuthorizationGateKind`, whose Windows platform quick-unlock value retains the stable `Hello` gate name. Missing configuration and invalid preference values fail closed to password setup/password unlock. The development-era profile adapter and implementation have been removed; synthetic legacy fixtures retain compatibility evidence without reintroducing the old model into production code.
...
- `PortableSettingsServiceTests` covers defaults, portable preference mapping, stable in-memory identity, retry after typed load failure, typed save failure, and structural exclusion of authorization material.
...
- The Release solution run covers the Windows-capable suite, Avalonia Headless, and Unix-specific tests. Permission-enforcement tests that cannot execute on Windows are explicitly skipped there and run on Unix CI hosts.
