...
- Stable Windows releases require a manually approved SignPath request whose GitHub origin is verified. CI holds a submitter-only `SIGNPATH_API_TOKEN`, never an exportable Foundation certificate, and verifies Authenticode plus product metadata before packaging.
...
- Conditional ownership is enabled only on Windows, macOS, and an available X11 display, matching Avalonia's documented in-process ownership support and its current Linux desktop backend. WSLg is detected through `DISPLAY` because it does not reliably populate `XDG_SESSION_TYPE`; unknown Linux sessions without X11 still advertise write-only capability and fail before copying. The data flow and security rule are unchanged: clearing occurs only while the exact in-process `DataTransfer` receipt remains current, and another clipboard owner prevents clearing. Adapter, policy, and scheduler tests cover WSLg/X11 detection, exact-receipt clearing, changed-clipboard preservation, unsupported-capability fail-closed behavior, and UI duration forwarding. No storage, secret, or compatibility format changes are involved.
- Selecting an account in Avalonia now matches the WPF workflow by generating and immediately copying that account's current code through the same conditional-clear service. Selection changes during generation cannot copy the superseded account's code, and periodic time-step refreshes update the display without repeatedly replacing the user's clipboard. Clipboard-policy opt-out and unsupported-platform behavior still fail before writing. Regression coverage verifies the selected code and bounded lifetime passed to the clipboard service and rejects stale-account copies.
...
- Credentialed M7 tag jobs require manually approved, origin-verified SignPath Authenticode signing on Windows and Developer ID signing, notarization, stapling, and Gatekeeper acceptance on macOS. The final job signs direct payloads, the aggregate manifest, and the separate Avalonia appcast with a pinned NetSparkle tool and verifies both metadata signatures before release upload. Key contents are no longer passed in appcast-generator process arguments.
