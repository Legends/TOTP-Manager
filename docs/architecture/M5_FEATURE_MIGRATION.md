...
Copy delegates to `IAsyncClipboardService`. When automatic clearing is enabled and supported, the service associates the scheduled clear with a platform receipt and clears only if clipboard ownership/content is unchanged. User replacement therefore is never overwritten. Plain copying requires only write capability; disabling automatic clear or running without conditional ownership no longer disables the core copy action, and the unsupported-clear fallback displays a localized warning.
...
The Avalonia Settings surface edits culture, logging threshold, idle timeout, session/minimize locks, automatic clipboard clearing and maximum lifetime, QR preview scale, interface-size override, and export-location behavior. Legacy encrypted-export and secret-visibility preference fields remain readable for settings-format compatibility but are not exposed: exports are always encrypted and secret inputs always start concealed. System DPI scaling remains the default; a bounded accessibility multiplier is applied before Avalonia platform initialization and therefore requires an application restart. Save validates codec bounds before persistence and restores the complete prior projection on expected or exceptional failure rather than partially retaining edited runtime values.

Code copying remains available whenever the platform supports text writes. Automatic clearing is enabled by default; when enabled and supported, the scheduled lifetime is the smaller of the code's remaining validity and the configured maximum. Unsupported conditional clearing falls back to a normal copy with an explicit warning, while write failures remain recoverable errors. QR scale updates the secret-bearing preview only after a successful settings save. Logging-level text explicitly notes that the bootstrap logger applies the new threshold after restart.
