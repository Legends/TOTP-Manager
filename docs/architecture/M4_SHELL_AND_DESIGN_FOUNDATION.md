...
Theme policy defaults to `RequestedThemeVariant="Dark"` to preserve the established WPF product identity rather than presenting a visually unrelated system-light shell. The dark semantic palette reuses the WPF navy surfaces, violet accent, blue selection, green highlight, compact typography, and control density. Theme-dependent resources are consumed through `DynamicResource`. A dedicated inherited high-contrast variant still overrides every semantic color automatically; real-target contrast testing remains open.

`AvaloniaThemeService` observes the platform color-values contract for the application lifetime. Normal contrast selects the product dark theme; `High` selects the custom `HighContrast` variant, whose palette uses black surfaces, white text/borders, yellow accents, cyan focus, explicit warning/error/success colors, and no card shadow. Color changes are applied on the UI dispatcher and the subscription is removed on exit. This completes behavior definition and automated policy coverage, but real-target contrast, focus visibility, forced-colors integration, and assistive-technology acceptance remain postponed target evidence.

The production shell consumes the tokens in the same compact hierarchy as WPF: narrow authenticator window, icon toolbar, searchable account list, selected issuer/code/countdown presentation, and right-side account editor. Tools and Settings retain explicit pages inside the same authorization boundary. The window starts at 320 by 520 device-independent pixels, has a bounded 300-pixel minimum width, and scrolls constrained content; the existing M3 target record defines 100%, 150%, and 200% scaling tests.

Selecting an account now starts code generation immediately, matching the WPF interaction instead of requiring a developer-style intermediate button. Generation captures the selected account identity and discards a late result when selection changes; it then generates for the newest selection. This prevents a stale account code from appearing after rapid navigation. Keyboard parity retains Ctrl+A, Ctrl+E, Ctrl+D, Ctrl+F, and adds Ctrl+L for explicit locking.
...
- Compatibility impact: high contrast follows the platform preference dynamically; normal contrast intentionally uses the established WPF dark visual identity on every target.
