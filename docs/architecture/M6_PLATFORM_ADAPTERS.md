...
Linux ships as a self-contained x64 portable tarball and an Ubuntu/Debian DEB. The DEB displays the OTP Harbor product name while retaining the `totp-manager` package ID, `/opt/totp-manager` installation root, launcher, desktop-file ID, and application-data paths for upgrade compatibility. It declares `libsecret-tools` plus native graphics/runtime prerequisites. AppImage is deferred until it has a maintained update, desktop-integration, and D-Bus policy; PKG is not selected on macOS because the app has no privileged daemon or system-wide payload.
